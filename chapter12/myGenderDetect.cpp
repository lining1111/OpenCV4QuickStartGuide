...
int main() {
    Mat img = imread(basePath + "faces.jpg");
    if (img.empty()) {
    ...
    //读取人脸识别模型
    String model_bin = basePath + "face_age/opencv_face_detector_uint8.pb";
    String config_text = basePath + "face_age/opencv_face_detector.pbtxt";
    Net faceNet = readNet(model_bin, config_text);

    //读取性别检测模型
    String genderProto = basePath + "face_age/gender_deploy.prototxt";
    String genderModel = basePath + "face_age/gender_net.caffemodel";
